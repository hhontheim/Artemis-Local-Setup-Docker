...
CHANGED_DIRS=$(git diff-tree --no-commit-id --name-only -r ${GITHUB_SHA} '*Dockerfile' '*RELEASE' | xargs -I {} dirname {} | uniq)  # Only matches, if `Dockerfile` or `RELEASE` is changed!
